...
public class ControleurSIALEController {
    ...
    public String create(@Valid ControleurSIALE controleurSIALE, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
    ...
    public String list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            ...
            uiModel.addAttribute("controleursiales", ControleurSIALE.findControleurSIALEEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) ControleurSIALE.countControleurSIALEs() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("controleursiales", ControleurSIALE.findAllControleurSIALEs(sortFieldName, sortOrder));
        }
        return "controleursiales/list";
    }

	@RequestMapping(method = RequestMethod.PUT, produces = "text/html")
    public String update(@Valid ControleurSIALE controleurSIALE, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, controleurSIALE);
	...
	void populateEditForm(Model uiModel, ControleurSIALE controleurSIALE) {
